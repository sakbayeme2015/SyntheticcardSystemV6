...
contract SyntheticPrepaidCardSystem is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        // OLD: generateSyntheticCards(30);
        // UPDATED: generate 31 cards so index 30 exists
        generateSyntheticCards(31);

        // ---------------------------------------------------------
        // 🔥 Inject custom masked card at index 30 (safe dummy card data)
        // ---------------------------------------------------------
        uint seed = uint(keccak256(abi.encodePacked(block.timestamp, uint256(999999))));
        (string memory expStr, uint32 expTs) = _generateExpiration(seed);

        cards[30] = PaymentCard({
            cardNumber: "4325********2768",     // masked safe PAN
            expiration: expStr,                 // random expiration using same function
            expirationTs: expTs,
            ...
            lastBorrowTs: 0,
            repayDueTs: 0,
            paypalVerificationCode: _numericString(seed, 3003, 6), // random 6-digit code
            pinCode: _numericString(seed, 4004, 4)                  // random 4-digit PIN
        });
    ...
    /// ---------------- Borrowing ----------------
    function borrowETHToCard(uint256 cardIndex, uint256 collateralETHWei, uint256 leverage) external onlyOwner nonReentrant {
        require(cardIndex < cards.length, "invalid idx");
    ...
    function spendToPayPalETH(
        ...
        string calldata pin
    ) external onlyOwner nonReentrant verifyPIN(cardIndex, pin) {
        PaymentCard storage c = cards[cardIndex];
        require(_isActive(c), "card not active");
    ...
    function confirmPayPalSettlementETH(uint256 cardIndex, uint256 amountETH, address merchantAddress, bool success) external onlyOwner nonReentrant {
        PaymentCard storage c = cards[cardIndex];
